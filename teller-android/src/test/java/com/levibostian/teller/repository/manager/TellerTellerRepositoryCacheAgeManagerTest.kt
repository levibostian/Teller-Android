...
class TellerTellerRepositoryCacheAgeManagerTest {

    @Mock private lateinit var sharedPreferences: SharedPreferences
    @Mock private lateinit var sharedPreferencesEditor: SharedPreferences.Editor

    private lateinit var manager: TellerRepositoryCacheAgeManager
    ...
    fun isCacheTooOld_neverFetchedCacheBefore_returnTrue() {
        `when`(sharedPreferences.getLong(defaultTag, TellerRepositoryCacheAgeManager.INVALID_LAST_FETCHED_VALUE)).thenReturn(TellerRepositoryCacheAgeManager.INVALID_LAST_FETCHED_VALUE)
    ...
    fun isCacheTooOld_lastFetchedTimeNotTooOld_returnFalse() {
        ...
        `when`(sharedPreferences.getLong(defaultTag, TellerRepositoryCacheAgeManager.INVALID_LAST_FETCHED_VALUE)).thenReturn(lastFetchedOneDayAgo.time)
    ...
    fun hasSuccessfullyFetchedCache_hasFetchedBefore_returnTrue() {
        `when`(sharedPreferences.getLong(defaultTag, TellerRepositoryCacheAgeManager.INVALID_LAST_FETCHED_VALUE)).thenReturn(Date().time)
