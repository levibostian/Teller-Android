...
interface OnlineDataStateListener<in DATA> {
     ...
     *
     * It's a best practice to show a UI to the user when data is done being fetched. Here is a tip for you: Sometimes in your app, a fetch can be very fast. It's not a good idea to show a UI to your user during fetching (when [fetchingFreshData] was called) that says something like "Syncing your profile..." and then when [finishedFetchingFreshData] gets called you change the UI to "Done!". What if that network call to fetch fresh data took 0.5 seconds? The user will see a message "Done!" and wonder, "What is done?!" It's recommended to use more descriptive messages such as "Your profile is up-to-date" or have a progress dialog that fades away to indicate the sync is complete and use the [cacheData] `fetched` parameter to show the user that data is only seconds old and has been fetched successfully. This may sounds complex but think of this while developing: If my user does not see the UI I show them when [fetchingFreshData] is called but they see the UI when [finishedFetchingFreshData] is called, will they be confused?
     *
